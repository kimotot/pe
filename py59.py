...
def crypt(key, code):
    """keyを暗号鍵として文字列を暗号・複合（どちらもXOR)する"""
    decode = []
    idx = 0
    keylen = len(key)

    while idx < len(code):
        decode.append(code[idx] ^ key[idx % keylen])
        idx += 1
...
def isreadable(code):
    """引数のコード列がアルファベットのコード範囲内であるか判定する"""
    for c in code:
        if 32 <= c <= 33 or \
            39 <= c <= 46 or \
            48 <= c <= 59 or \
...
if __name__ == "__main__":
    ...
    @basic.time_log
    def test():
        decode = crypt([79,59,12],CRYPTCODE)
        print(codetostring(decode))
        print(isreadable(decode))
    ...
    def main():
        ...
        for key in keys:
            decode = crypt(key,CRYPTCODE)

            if isreadable(decode):
                print(codetostring(decode))
                print(key)
                print(sum(decode))
